...
from app.models.schemas import StrategyInput, StrategyResponse, HistoryResponse, FeedbackRequest
from app.dependencies.auth import get_current_user
from app.services.strategy_service import strategy_service
from app.services.usage_service import usage_service
from app.core.rate_limit import limiter, get_rate_limit_ai
...
@limiter.limit(get_rate_limit_ai)
...
async def submit_feedback(
    ...
    result = await strategy_service.submit_feedback(
...
@router.post("/strategies/{strategy_id}/blueprint")
async def get_blueprint(
    strategy_id: str,
    current_user: dict = Depends(get_current_user)
):
    blueprint = await strategy_service.get_blueprint(strategy_id, current_user["id"])
    if not blueprint:
        raise HTTPException(status_code=404, detail="Strategy not found")
    return blueprint
